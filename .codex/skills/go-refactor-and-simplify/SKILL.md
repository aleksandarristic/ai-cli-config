---
name: go-refactor-and-simplify
description: Refactor and simplify Go code in a moderately complex repository while preserving behavior and improving maintainability/performance where justified.
---

## Purpose
Make targeted refactors that reduce complexity and future-proof a Go codebase without changing externally observable behavior unless explicitly requested.

## Operating principles
- Preserve behavior by default. If behavior must change, call it out explicitly and minimize blast radius.
- Prefer small, reviewable changes over large rewrites.
- Match existing repo conventions (style, error handling, context usage, logging, structure).
...
1) Entry points (cmd/ binaries, main packages, service runners), primary workflows, and critical packages.
2) Code ownership boundaries: core domain logic vs adapters (I/O, HTTP, DB, queue, CLI).
3) Existing conventions: config patterns, dependency injection, context usage, logging, error handling.
4) Risks: implicit coupling, shared mutable state, hidden global config, unbounded goroutines.
...
- Introduce small interfaces for cross-package contracts
