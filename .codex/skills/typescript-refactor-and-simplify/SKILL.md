---
name: typescript-refactor-and-simplify
description: Refactor and simplify TypeScript code in a moderately complex repository while preserving behavior and improving maintainability/performance where justified.
---

## Purpose
Make targeted refactors that reduce complexity and future-proof a TypeScript codebase without changing externally observable behavior unless explicitly requested.

## Operating principles
- Preserve behavior by default. If behavior must change, call it out explicitly and minimize blast radius.
- Prefer small, reviewable changes over large rewrites.
- Match existing repo conventions (style, async patterns, typing, logging, error handling, structure).
...
1) Entry points (CLI/bin, server startup, build scripts), primary workflows, and critical modules.
2) Code ownership boundaries: core domain logic vs adapters (I/O, HTTP, DB, queue, CLI).
3) Existing conventions: config patterns, dependency injection, async model, logging, error handling, type style.
4) Risks: implicit coupling, shared mutable state, hidden global config, unbounded async concurrency.
...
- Normalize error paths and boundaries
- Introduce small interfaces for cross-module contracts
...
- Keep functions small; reduce nesting; flatten control flow where it improves clarity.
